...
5. **Delete Order on Empty**:
   - If the quantities of all products in the order are set to `0` (or omitted), the entire order is deleted from the database.

### Responses

#### Success (200 OK) - Order Updated
...
#### Success (200 OK) - Order Deleted (Empty)
If all products are returned, the order is deleted from the database and `data` is returned as `null`.
```json
{
  "statusCode": 200,
  "success": true,
